...
--14
CREATE TABLE airbnb_data_usa AS
SELECT *, 'United States' AS country
FROM airbnb_data 

--15
CREATE TABLE airbnb_data_country AS
SELECT
	country
FROM airbnb_data_usa;
...
--17
CREATE TABLE reviews_summary_by_room_type AS
SELECT
    room_type,
    COUNT(number_of_reviews) AS total_number_of_reviews,
    AVG(review_rate_number) AS avg_review_rate
FROM airbnb_data
GROUP BY room_type
